...
import { BookOpen, Brain, FileText, Layers3, Plus, X } from 'lucide-react';
import type { Difficulty, LibraryCourse, LibraryFlashcard, LibraryNote, LibraryTopic, Visibility } from '../types';
...
export function LibraryBoard({ courses, topics, notes, flashcards, onCreateCourse, onCreateTopic, onCreateNote, onCreateFlashcard }: LibraryBoardProps) {
  ...
  const topicId = selectedTopicId || courseTopics[0]?.id || '';
  ...
  }), [courses.length, topics.length, notes.length, flashcards.length]);
    ...
    <div className="space-y-5">
      <NeonCard tone="green">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-2xl font-black text-white">Biblioteca personal</h2>
              <p className="text-sm text-slate-400">Cursos, temas, apuntes y flashcards para estudiar con disciplina.</p>
            </div>
          </div>
          <div className="grid grid-cols-4 gap-2 text-center">
            <SmallTotal label="Cursos" value={totals.courses} />
      ...
      <div className="grid gap-5 xl:grid-cols-[0.8fr_1.2fr]">
        <NeonCard tone="blue">
          ...
          <div className="mt-4 space-y-2">
            {courses.length ? courses.map((course) => (
              <button
                key={course.id}
                className={`w-full rounded-md border p-3 text-left transition ${course.id === courseId ? 'border-forge-blue/40 bg-forge-blue/10' : 'border-white/10 bg-black/25 hover:border-forge-blue/30'}`}
                onClick={() => {
              ...
              >
                ...
                <p className="mt-1 text-sm text-slate-400">{course.description || 'Sin descripción'}</p>
              </button>
        ...
        <NeonCard tone="violet">
          <PanelHeader title="Temas" action="Nuevo tema" disabled={!courseId} onClick={() => setModal('topic')} />
      ...
      <div className="grid gap-5 xl:grid-cols-2">
        <NeonCard tone="blue">
          <PanelHeader title="Apuntes" action="Nuevo apunte" disabled={!topicId} onClick={() => setModal('note')} />
        ...
        <NeonCard tone="green">
          <PanelHeader title="Flashcards" action="Nueva flashcard" disabled={!topicId} onClick={() => setModal('flashcard')} />
          <div className="mt-4 grid gap-3 md:grid-cols-2">
