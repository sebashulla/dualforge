...
import { AlertTriangle, CheckCircle2, RotateCcw, Send, SquareCheckBig, XCircle } from 'lucide-react';
...
export function ChallengeCenter({
  ...
  return (
    <div className="grid gap-5 xl:grid-cols-[0.9fr_1.1fr]">
      <ChallengeForm disabled={!duelId || !rival} rival={rival} onCreateChallenge={onCreateChallenge} />
      <NeonCard tone="red">
...
function ChallengeForm({ disabled, rival, onCreateChallenge }: { disabled: boolean; rival: Profile | null; onCreateChallenge: (challenge: NewChallenge) => Promise<void> }) {
  ...
  async function submit(event: FormEvent) {
    ...
    if (!rival || unsafe) return;
    setLoading(true);
    await onCreateChallenge({ ...challenge, target_user_id: rival.id });
    setChallenge((current) => ({ ...current, title: '', description: '', creator_comment: '' }));
    setLoading(false);
  }
  ...
  return (
    <NeonCard tone="violet">
      ...
      <div className="mt-3 rounded-md border border-forge-blue/20 bg-forge-blue/10 p-3 text-xs text-slate-300">
      ...
      <form onSubmit={submit} className="mt-4 space-y-3">
        ...
        <button className="w-full rounded-md bg-forge-violet px-4 py-3 font-bold text-white disabled:opacity-50" disabled={disabled || loading || unsafe}>
          {disabled ? 'Necesitas un rival activo' : loading ? 'Enviando...' : 'Enviar reto'}
        </button>
      </form>
    </NeonCard>
  );
